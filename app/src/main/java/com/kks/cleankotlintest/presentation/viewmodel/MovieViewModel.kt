...
class MovieViewModel(
    ...
    private lateinit var _screenState: MutableLiveData<ScreenState<DataState<List<MovieVO>>>>
    ...
    val screenState: LiveData<ScreenState<DataState<List<MovieVO>>>>
        get() {
            ...
            if (movies.isNullOrEmpty()) {
                when {
                    ...
                    page == 1 -> _screenState.postValue(
                        ScreenState.Render(DataState.Error("NO_NETWORK"))
                    )
            ...
            try {
                ...
                resultFlow.collect {
                    when (it) {
                        ...
                        is DataState.GenericError -> _screenState.postValue(
                            ScreenState.Render(
                                DataState.Error(
                                    if (it.error.isNullOrEmpty())
                                        it.code.toString()
                                    else "Error code: ${it.error}"
                                )
                            )
                        )
                        DataState.NetworkError -> _screenState.postValue(
                            ScreenState.Render(DataState.Error("NO_NETWORK"))
                        )
                    }
                }
                ...
                _screenState.postValue(
                    ScreenState.Render(DataState.Error("UNKNOWN_ERROR"))
                )
            }
        }
